...
def test_template_mark_integrity_reasons_detects_mismatch():
    ...
    assert any("manifest_missing_qids:Q3" == r for r in reasons)


def test_template_mark_integrity_reasons_detects_missing_and_unplaced():
    reasons = _template_mark_integrity_reasons(
        expected_qids=["Q1", "Q2", "Q3"],
        graded_qids=["Q1", "Q2", "Q3"],
        marks_placed=1,
...
def test_template_mark_integrity_reasons_detects_unknown_and_duplicate_qids():
    reasons = _template_mark_integrity_reasons(
        expected_qids=["Q1", "Q2"],
        graded_qids=["Q1", "Q9", "Q9"],
        marks_placed=2,
        marks_skipped_missing=1,
        unplaced_items=[],
    )
    assert any("manifest_unknown_qids:Q9" == r for r in reasons)
    assert any("manifest_duplicate_qids:Q9" == r for r in reasons)
